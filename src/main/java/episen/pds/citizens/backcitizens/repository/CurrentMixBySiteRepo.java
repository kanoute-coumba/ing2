...
public interface CurrentMixBySiteRepo extends CrudRepository<MixEnBySite,Integer> {
    ...
    @Query(value = "select mix,t2.address,t2.type_building,active_equip,number_equip from " +
            ...
            "(select sum(value) as mix, address, count(c.id_central) as active_equip from current_production p " +
            ...
            "( select p1.id_production from current_production p1 " +
            "where date_time = (select max(date_time) from current_production p2 group by p2.id_central having p1.id_central = p2.id_central)) " +
