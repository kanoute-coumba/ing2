...
public interface EquipmentRepo extends CrudRepository<Equipment, Integer> {
    ...
    @Query(value = "select distinct m.value, s.type from measure m inner join sensor s on m.id_sensor = s.id_sensor inner join room r on s.id_room = r.id_room inner join equipment e on r.id_room = e.id_room inner join equipment_data ed on e.id_equipment = ed.id_equipment_data where s.type = 'capteur de température' and m.timestamp between cast(:date1 as timestamp )   and cast(:date2 as timestamp )", nativeQuery = true)
     Map<String, String> valueSensor (@Param("date1") String date1, @Param("date2") String date2);
